...
class AwareDecoder(nn.Module):
    def __init__(self,
        ...
        self.const_vector = nn.Embedding(*const_vector.size())  # [N_C, H] or [N_C, H*2] regarding concat
        #assert self.const_num == self.const_vector.size(0)

        # operator candidate vector
        # N_O : number of operator
        self.operator_vector = nn.Embedding(*operator_vector.size()) # [N_O, H] or [N_O, H*2] regarding concat
